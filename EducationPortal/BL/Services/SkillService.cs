...
namespace Application.Services
{
    public class SkillService : IServiceSkill
    {
        ...
        {
            return mapper.GetMapper().Map<IEnumerable<Skill>,IEnumerable<SkillDTO>>(repository.GetAll<Skill>());
        }
        ...
        {
            var skillsId = repository.GetAllBy<CompositionSkillCourse>(i => i.CourseId == idCourse)
                ?.Select(i => i.SkillId);
            return GetAllBy(i => skillsId?.Contains(i.Id)??false);
        }
